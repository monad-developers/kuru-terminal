...
async function fetchEvents(contractAddress: string, fromBlock: number): Promise<RawLog[]> {
  try {
    ...
    while (hasMoreData && currentPage < maxPages) {
      ...
      try {
        ...
        if (data && data.length > 0) {
          // Process events - update highest block seen and filter duplicates
          const newEvents: RawLog[] = [];
          for (const event of data) {
            // Update highest block seen
          ...
          logger.info(`Fetched page ${currentPage} for contract ${contractAddress}: ${data.length} events (${newEvents.length} new, ${data.length - newEvents.length} duplicates, ${Math.round(duplicationRatio * 100)}% duplication)`);

          // Check if we've reached standard max pages but haven't progressed beyond starting block
          if (currentPage >= STANDARD_MAX_PAGES - 1 && highestBlockSeen === fromBlock && seenEvents.size < 5) {
            // We're likely in a situation where we need to paginate more to find events from higher blocks
            if (maxPages === STANDARD_MAX_PAGES) {
              maxPages = EXTENDED_MAX_PAGES;
              logger.warn(`Detected potential block stagnation at block ${fromBlock} for contract ${contractAddress}. Only ${seenEvents.size} unique events found after ${STANDARD_MAX_PAGES} pages. Extending pagination limit to ${EXTENDED_MAX_PAGES} pages to search for higher blocks.`);
            }
          }
    ...
    if (currentPage >= maxPages) {
      if (maxPages === EXTENDED_MAX_PAGES) {
        logger.warn(`Reached extended maximum page limit (${EXTENDED_MAX_PAGES}) for contract ${contractAddress}. Highest block seen: ${highestBlockSeen}. Total unique events: ${seenEvents.size}.`);
      } else {
        logger.warn(`Reached maximum page limit (${STANDARD_MAX_PAGES}) for contract ${contractAddress}. Highest block seen: ${highestBlockSeen}. Total unique events: ${seenEvents.size}.`);
      }
    }

    logger.info(`Completed fetching all ${allEvents.length} events for contract ${contractAddress} from block ${fromBlock}. Highest block seen: ${highestBlockSeen}`);
    return allEvents;
  } catch (error) {
